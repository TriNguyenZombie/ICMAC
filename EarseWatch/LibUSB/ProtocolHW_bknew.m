...
static NSMutableData *senddata(int fileDescriptor,Byte *arr, int numbyte,int *sizeout)
{
    ...
    for (tries = 0; tries < kNumRetries; tries++)
    {
        ...
        *bufPtr = '\0';
        ...
        // printf("Read \"%s\"\n", logString(buffer));
        //NSLog(@"senddata: firstdata.bytes: %@", firstdata);
        //NSLog(@"senddata: result: %hhu", result);
        printf("\n");
...
- (NSDictionary *) checkVersion:(NSString *)serial
{
    ...
    NSLog(@"READ VERSION: result: %@", result);
...
{
    printf("%s [ledControl] Board serial: %s\n",__func__, [serial UTF8String]);
...
- (unsigned short) usbControlTurnOFF:(NSString *)serial usbArr:(Byte*)arr
{
    ...
    if([self openport:serial]==FALSED)
    {
        NSLog(@"[ProtocolHW][usbControlTurnON_OFF] open port failed \n" );
        return FALSED;
    }
...
unsigned short ledControl(unsigned char LEDMode[8],int fileDescriptor)
{
   ...
   // unsigned short timeOut = 3000;
    ...
    NSLog(@"Len of led control: %hu", len);
    ...
    NSMutableData *data = [NSMutableData dataWithBytes:dataSend length:len];
    NSLog(@"LED CONTROL: data: %@", data);

    int sizeRead = 0;
    NSMutableData *result = sendData(fileDescriptor,data);
    NSLog(@"LED CONTROL: result: %@", result);
...
//- (bool) upgradeFirmware:(NSString*)firmwarePath
//{
//    NSLog(@"Upgrade firmware: %d", fileDescriptor);
//    int ret = 0;
//
//    char * name = NULL;
//    long size = 0, countbyte = 0;
//    unsigned short addr = 0;
//
//    unsigned char* buff = NULL;
//    uint32_t crc32sum = 1;
//    uint32_t crc32verify = 0;
//    unsigned short moduleID = 0;
//    uint32_t fw_version = 0;
//    unsigned short res = 0;
//    long max_len = 2048;
//    size_t size_read = max_len, cnt = 0;
//    int size_crc = 0;
//    FILE* file = NULL;
//    long i, count;
//    unsigned char data;
...
//
//    moduleID = 0;
//    name = (char *)[firmwarePath UTF8String];
//    file = fopen(name, "rb");
//
//    if (file == NULL)
//    {
//        NSLog(@"Could not open file: %@", file);
//        return false;
//    }
//
//    fseek(file, 0, SEEK_END);
...
//    if(size < 0)
//    {
//        NSLog(@"No file: %ld", size);
//        return false;
//    }
//
//    if(size > 700000)
//    {
//        NSLog(@"File large: %ld", size);
//        return false;
//    }
//
//    buff = (unsigned char*) malloc(max_len);
...
//        if (cnt != (unsigned int) size_read)
//        {
//            fprintf(stderr, "Error reading filesystem %d - %d - %d\n", (unsigned int)cnt, (unsigned int)size_read, (unsigned int)i);
//            fclose(file);
//            return false;
//        }
//        //kiem tra block data cuoi cung cua file
...
//    for(int i = 0; i < 1; i++)
//    {
//        res = upgradeFw(moduleID, fileDescriptor);
//        if(res == FWSUCCESS)
//        {
//            break;
//        }
...
//    sleep(2);
//
//    res = startUpgradeFw(moduleID, fw_version, size - 4, crc32sum, fileDescriptor);
//
//    if(res!= FWSUCCESS)
//    {
//        res = startUpgradeFw(moduleID, fw_version, size - 4, crc32sum, fileDescriptor);
...
//        if (cnt != (unsigned int) size_read)
//        {
//            fprintf(stderr, "[2] Error reading filesystem %d - %d - %d\n", (unsigned int)cnt, (unsigned int)size_read, (unsigned int)i);
//            fclose(file);
//            return false;
//        }
//        //kiem tra block data cuoi cung cua file
...
//        }
//
//        res = sendUpgradeFw(moduleID, addr, buff, size_read, fileDescriptor);
//
//        if(res != FWSUCCESS)
//        {
//            sleep(1);
...
////    }
////
////    res = finishUpgradeFw(moduleID, fileDescriptor);
////
////    if(res!= FWSUCCESS)
////    {
////        res = finishUpgradeFw(moduleID, fileDescriptor);
